...
resource "tls_cert_request" "tfe_server" {
  ...
  subject {
    common_name  = "${var.datacenter}-server-tfe.${var.domain}"
    organization = "msk"
  }
}
...
data "template_file" "user_data" {
  ...
  vars = {
    hostname                = "${var.datacenter}-tfe.${var.domain}"
    tfe_admin_user_config   = base64encode(data.template_file.tfe_admin_user_config.rendered)
    replicated_conf         = base64encode(data.template_file.replicated_conf.rendered)
    tfe_settings            = base64encode(data.template_file.tfe_settings.rendered)
    ...
    tfe_lic_file            = filebase64("~/workspace/marc-steffen-kaesz---mkaeszhashicorpcom---emea-tam.rli")
  }
}
...
inline = [
  ...
  "sudo ./install.sh airgap no-proxy private-address=$ip public-address=$ip > /dev/null 2>&1",
  "while ! curl -ksfS --connect-timeout 5 https://${self.name}.${var.domain}/_health_check; do sleep 5; done",
  "sudo replicated admin --tty=0 retrieve-iact > iact.txt",
  "curl -k --header 'Content-Type: application/json' --request POST --data @'/opt/tfe/config/tfe-admin-user.json' https://${self.name}.${var.domain}/admin/initial-admin-user?token=$(cat iact.txt)"
]
}
